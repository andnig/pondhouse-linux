...
grep -Fq 'PACKAGE_VERSION=${PONDHOUSE_PACKAGE_VERSION:-2026.08.15-35}' "$COMMAND" || fail "production package release is pinned"; pass "production package release is pinned"
grep -Fq 'PACKAGE_SHA256=${PONDHOUSE_PACKAGE_SHA256:-7c80d22cdcf4e7e27fd0164685f2aa5be1e5e56baec30201f8debaed1408c964}' "$COMMAND" || fail "production package checksum is pinned"; pass "production package checksum is pinned"
...
grep -Fq 'REPOSITORY_SHA256=${PONDHOUSE_REPOSITORY_SHA256:-c4d3f5df4c49bf898975e90a994be0687a1feaf0b271a51bb02d142703d563cd}' "$COMMAND" || fail "production repository checksum is pinned"; pass "production repository checksum is pinned"
...
grep -Fq 'Automatic workspace application startup is currently unreliable' <<<"$complete_output" || fail "migration omits autostart reliability guidance"; pass "migration explains autostart reliability"
grep -Fq 'autostartworkaround.lua and matching commented' <<<"$complete_output" || fail "migration omits manual autostart template"; pass "migration references manual autostart template"
grep -Fq 'enable its commented SUPER+CTRL+I manual launch' <<<"$complete_output" || fail "migration prompt omits manual launch binding"; pass "migration prompt enables manual autostart binding"
grep -Fq 'do not put those applications in autostart.lua' <<<"$complete_output" || fail "migration prompt still recommends Hyprland autostart"; pass "migration prompt avoids unreliable autostart"
